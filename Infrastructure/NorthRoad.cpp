...
NorthRoad::NorthRoad()
{
    ...
    for (uint8 i = 0; i < my_totalLanes; i++)
    {
        if (i < intersection_params.ns_number_of_exits) //must be defining an exit lane
        {
            ...
            my_laneList[i] = new ExitLane(i, NORTH, NULL_PATH); 
        }
        else //we are now defining entry lanes
        {
            if(intersection_params.ns_number_of_entries == 1)
            {
                ...
                my_laneList[i] = new EntryLane(i, NORTH, ALL_PATHS); 
            }
            else if ( intersection_params.ns_number_of_entries == 2)
            {
                ...
                my_laneList[i] = new EntryLane(i, NORTH, (i - intersection_params.ns_number_of_exits) % 2 ? STRAIGHT_LEFT : STRAIGHT_RIGHT);
            }
            else 
            {
                ...
                if (current_entry == 0)
                {
                    ...
                    my_laneList[i] = new EntryLane(i, NORTH, LEFT);
                }
                else if (i + 1 == my_totalLanes)
                {
                    //furthest right lane must go right
                    my_laneList[i] = new EntryLane(i, NORTH, RIGHT);
                }
                else
                {
                    //all lanes that aren't furthest left or right must go straight
                    my_laneList[i] = new EntryLane(i, NORTH, STRAIGHT);
                }
            }
        }
    }
